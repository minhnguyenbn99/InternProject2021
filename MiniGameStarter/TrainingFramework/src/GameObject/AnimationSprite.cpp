...
AnimationSprite::AnimationSprite() : m_numFrames(0), m_currentFrame(0), m_frameTime(0), m_currentFrameTime(0), m_setcoin(false)
{
}
AnimationSprite::AnimationSprite(std::shared_ptr<Model> model, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture, int numFrames, float frameTime ) 
	: Sprite2D(model, shader, texture), m_numFrames(numFrames), m_currentFrame(0), m_frameTime(frameTime), m_currentFrameTime(0), m_setcoin(false)
{
}
...
void AnimationSprite::SetCoin(bool setcoin) {
	m_setcoin = setcoin;
}
bool AnimationSprite::GetCoin() {
	return m_setcoin;
}